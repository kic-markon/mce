...
(defparameter x nil)
(defparameter *call-pos* nil "Coordinates of passenger indicators")
...
(defparameter *passenger-plane* nil)
...
(defparameter *call-column* 2)
(defparameter *shaft-column* 3)
(defparameter *button-column* 1)
...
(defmethod disp-call((p passg))
  ...
  (with-slots (from to view id sid dir) p
    ...
    (multiple-value-bind (y0 x0) (values-list (aref *call-pos* from ncall))
      (multiple-value-bind (y1 x1) (values-list (aref *call-pos* to ncall))
        (let ((begin (ltk:create-oval (view *passenger-plane*) (- x0 dd)(- y0 dd)(+ x0 dd)(+ y0 dd)))
...
(defun start()
  ...
  (setq *passenger-plane* (make-traff 1 12))
  ...
  (dotimes (i (top *bldg*)) 
    (let ((tx (ltk:create-text (view *passenger-plane*) (- left 12) (+ (* (- *top+* i *nc* 1) d) p) (format nil "~A" i))))
      (ltk:itemconfigure (view *passenger-plane*) tx :fill "powder blue")))
  
  ;; Set position data for calls at each floor
  (dotimes (i *top+*) 
    (dotimes (j *call-num*)
      (setf (aref *call-pos* (- *top+* i 1) j)
            (list (* (+ (- i *nc*) 0.5) d) (* j r)))))
  ...
  (setq b0 (make-instance 'ltk::button :master nil :text "CALL"
             :command (lambda () 
